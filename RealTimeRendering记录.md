...
对于一个像素着色器不能获取或是影响邻接像素的结果的规则有一些例外。其中一个就是像素着色器可以在梯度或导数信息的计算过程中，立即访问相邻片段的信息(尽管是间接的)。像素着色器提供了任意插值值沿x和y屏幕轴每像素的变化量，这些值对于各种计算和纹理寻址都很有用。这些梯度对于纹理过滤等操作特别重要，在这些操作中，我们想知道一张图像对一个像素的覆盖程度。所有现代gpu都是通过2x2分组（被称为一个quad）处理片段来实现这一特性的。当一个顶点着色器请求一个梯度值的时候，返回相邻片段之间的差值。参考图3.15.![fig3-155](.\fig3-155.png) 

一个统一的核心具有访问邻近数据的能力——在相同的warp中但是保持在不同的线程中——因此可以计算像素着色器中使用的梯度 。这种实现的一个后果是，在受动态流控制影响的部分着色器中，梯度信息不能被访问，举个例子，一个“if”判断或者是需要进行多次迭代的循环，。一组中的所有片段必须使用相同的指令集进行处理，以便所有四个像素的结果对梯度计算都有意义。这是一个即使在离线渲染系统中也存在的一个基本限制
